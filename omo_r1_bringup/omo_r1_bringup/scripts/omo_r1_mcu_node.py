...
class OMOR1MiniNode(Node):
  def __init__(self):
    ...
    self.declare_parameters(
      ...
      parameters=[
        ...
        ('motor.gear_ratio', 15.),
        ('motor.max_lin_vel_x', 2.),
        ('sensor.enc_pulse', 1000.),
      ])
    ...
    self.gear_ratio = self.get_parameter_or('motor.gear_ratio', Parameter('motor.gear_ratio', Parameter.Type.DOUBLE, 15.)).get_parameter_value().double_value
    ...
    self.enc_pulse = self.get_parameter_or('sensor.enc_pulse', Parameter('sensor.enc_pulse', Parameter.Type.DOUBLE, 1000.)).get_parameter_value().double_value
    ...
    self.distance_per_pulse = 2*math.pi*self.wheel_radius / (self.enc_pulse * self.gear_ratio * 4)
  ...
  def update_odometry(self, enc_l, enc_r):
    enc_left_now = enc_l
    enc_right_now = enc_r
    ...
    #   %(enc_l, enc_r, trans_vel, orient_vel, 
  ...
  def update_robot(self):
    ...
    enc_l = self.ph._enc[0]
    enc_r = self.ph._enc[1]
    odo_l, odo_r = self.convert2odo_from_each_wheel(enc_l, enc_r)
